...
Process {
	...
	If ($NoPercent) {
		$Bar += New-DrawBar -Char "[ " -Draw $DrawBar
	}
	Else {
		If     ($Percent -eq 100) {$Bar += New-DrawBar -Char "$Percent% [ " -Draw $DrawBar}
		ElseIf ($Percent -ge 10)  {$Bar += New-DrawBar -Char " $Percent% [ " -Draw $DrawBar}
		Else                      {$Bar += New-DrawBar -Char "  $Percent% [ " -Draw $DrawBar}
	}
	
	For ($i=1; $i -le ($BarValue = ([Math]::Round($Percent * $BarLength / 100))); $i++) {
	
		If     ($i -le ($GreenBorder * $BarLength / 100))  {$Bar += New-DrawBar -Char ($BarChars.Char1) -Color 'DarkGreen' -Draw $DrawBar}
		ElseIf ($i -le ($YellowBorder * $BarLength / 100)) {$Bar += New-DrawBar -Char ($BarChars.Char2) -Color 'Yellow' -Draw $DrawBar}
		Else                                               {$Bar += New-DrawBar -Char ($BarChars.Char3) -Color 'Red' -Draw $DrawBar}
	}
	For ($i=1; $i -le ($EmptyValue = $BarLength - $BarValue); $i++) {$Bar += New-DrawBar -Char ($BarChars.Char4) -Draw $DrawBar}
	$Bar += New-DrawBar -Char " ]" -Draw $DrawBar
